    ...
    return (
      <View style={styles.invoiceCard}>
            ...
            {invoice.serviceType && (
              <View style={styles.serviceTypeContainer}>
                <View style={[styles.serviceTypeBadge, {
                  backgroundColor: invoice.serviceType === 'OP' ? '#E0F2FE' : 
                                   invoice.serviceType === 'IP' ? '#FEF3C7' : '#F3E8FF'
                }]}>
                  <Text style={[styles.serviceTypeText, {
                    color: invoice.serviceType === 'OP' ? '#0369A1' : 
                           invoice.serviceType === 'IP' ? '#D97706' : '#7C3AED'
                  }]}>
                    {invoice.serviceType}
                  </Text>
                </View>
                {invoice.paymentId && (
                  <Text style={styles.linkedPaymentText}>
                    Linked to Payment
                  </Text>
                )}
              </View>
        ...
        <View style={styles.invoiceDetails}>
          ...
          {invoice.isAutoGenerated && (
            <View style={styles.autoGenDetails}>
              <Text style={styles.autoGenTitle}>Auto-Generated Invoice</Text>
              <View style={styles.autoGenRow}>
                <Text style={styles.autoGenLabel}>Generated From:</Text>
                <Text style={styles.autoGenValue}>
                  {invoice.generatedFrom === 'payment' ? 'Payment Addition' : 'Patient Registration'}
                </Text>
              </View>
              {invoice.originalId && (
                <View style={styles.autoGenRow}>
                  <Text style={styles.autoGenLabel}>Source ID:</Text>
                  <Text style={styles.autoGenSourceId}>{invoice.originalId}</Text>
                </View>
              )}
              {invoice.generatedAt && (
                <View style={styles.autoGenRow}>
                  <Text style={styles.autoGenLabel}>Generated At:</Text>
                  <Text style={styles.autoGenValue}>
                    {invoice.generatedAt.toDate ? 
                      invoice.generatedAt.toDate().toLocaleString() : 
                      invoice.generatedAt}
                  </Text>
                </View>
              )}
