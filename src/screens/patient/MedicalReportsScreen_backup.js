...
const MedicalReportsScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    if (!userData?.id && !userData?.uid) {
      console.log('⚠️ No user ID available for reports');
      setLoading(false);
      return;
    }
    ...
    try {
      // Query reports sent to this specific user
      ...
      const unsubscribe = onSnapshot(reportsQuery, (snapshot) => {
        ...
        // Fallback to context reports if available
        const fallbackReports = contextReports?.filter(r => 
          r.patientId === userId && r.sentToPatient
        ) || [];
